...
### 6.6 评分体系（为后续加分/扣分预留空间）

[`config.py`](config.py)：

- `RAW_SCORE_MAX = 20`：LLM 直接产出的原始分上限（relevance + novelty）
- `TOTAL_SCORE_MAX = 30`：**最终总分上限**，预留 +10 的加分项 budget（作者 / 顶会 / 降权等）
- `DEEP_ANALYSIS_MIN_TOTAL_SCORE = 21`：深度精读阈值，对应新总分的 70%
- `TOP_DISPLAY_MIN_SCORE = 18`、`TOP_DISPLAY_MIN_COUNT = 5`：日报"重点论文"展示阈值 + 保底数

后续 PR（作者加分、顶会加分、黑名单降权）会在 stage2 完成后、reporter 展示前做一次"分数叠加"，不改 LLM 端口。
...
- 排序相关：`MAX_PAPERS_TO_RANK` / `TOPICS_OF_INTEREST` / `RAW_SCORE_MAX` / `TOTAL_SCORE_MAX`
...
13. **新增的评分叠加（作者 / 顶会 / 降权）放在 stage2 之后**，不要改 LLM 输出维度
