...
- [`score_adjust.py`](score_adjust.py)：stage2 之后的规则型评分叠加层，纯正则 + 字典匹配，不碰 LLM
  - `apply_score_adjustments(papers)`：整批算 `author_bonus` / `venue_bonus` / `penalty`、按 `BONUS_BUDGET` 封顶、按新 `total_score` 重排 `rank`
  ...
  - `_match_blacklist_keywords`：扫 `title + abstract`，word-boundary + 可选复数 `s?` 匹配 `BLACKLIST_KEYWORDS`，命中累加 `PENALTY_PER_HIT`（封顶 `PENALTY_CAP`）
...
**关键决策**：粗筛之后的候选集即 **selected papers**。它们会同时进入机构推理、摘要精排，并以评分明细写入 `selected_papers` 快照，不再固定截断到 TOP 10。

### 6.2 规则预筛

[`_rule_prefilter`](ranker.py) 两步纯字符串规则，零成本：
...
2. **关键词白名单命中**：拼接 `title + abstract + categories`，小写匹配 `PREFILTER_KEYWORDS`。命中即保留；全量未命中时 fallback 取前 `STAGE1_KEEP * 2 = 60` 篇兜底（已剔除黑名单）。

注意：

- **不应用 `MAX_PAPERS_TO_RANK` 截断**——这是零成本的纯字符串匹配，LLM 限额只在下一步生效
...
**为什么全量评分**：`selected_papers` 快照承担"候选集回放 + 评分审计 + 未来重排"三个职责，必须包含所有评分明细；日报的 TOP N 交给 reporter 层按阈值动态挑。
...
- `BONUS_BUDGET = 10`：`author_bonus + venue_bonus` 之和硬封顶（不含 `penalty`）
...
- 配置：`BLACKLIST_KEYWORDS = ["Drone Racing", "V2V", "V2X", "Remote Sensing", "UAV", "Quadrotor"]`
- 扫描范围：`paper.title + paper.abstract`（不扫 comments / 机构 / 评论区）。**不剔除候选**，只压分，保留审计痕迹。
- 匹配规则（`_build_keyword_pattern`）：
  - case-insensitive + **word boundary**，避免 `UAVNet` 误伤 `UAV`、`V2XNet` 误伤 `V2X`
  - 尾部允许 `s?`：`UAV` / `UAVs` 都算命中，`Quadrotor` / `Quadrotors` 都算
  - 词组内部空格用 `\s+`：`Drone Racing` 命中 `Drone  Racing`（双空格）与换行拼接
- 每命中一个关键词扣 `PENALTY_PER_HIT = 6`，单篇 `penalty` 累计封顶 `PENALTY_CAP = 12`（多个关键词撞车不会把分打到极端负数）。
- **penalty 不受 `BONUS_BUDGET` 限制**：bonus 再高也能被降权覆盖；但最终 `total_score` 仍 clamp 到 `[0, TOTAL_SCORE_MAX]`，不会变负。
- 和"subject 黑名单"的分工：`BLACKLIST_SUBJECTS` 在规则预筛阶段**一刀切剔除**（不浪费 LLM），`BLACKLIST_KEYWORDS` 在 stage2 之后**降权但保留**（便于审计 + 允许 featured author 仍然把分抬上来）。

#### 封顶策略

1. `author_bonus` 先各自封顶到 `AUTHOR_BONUS_CAP`
2. `author_bonus + venue_bonus` 超出 `BONUS_BUDGET = 10` 时，**先压 venue，再压 author**（保护"重点作者"信号优先级）
3. `penalty` 按 `PENALTY_CAP` 封顶（不受 `BONUS_BUDGET` 限制）
4. `total_score` 最后再按 `[0, TOTAL_SCORE_MAX = 30]` 硬截断

#### 展示

- `selected_papers` 快照新增 `作者加分 / 顶会加分 / 降权` 三列，0 显示为 `-`
- 日报"重点论文详细卡片"里：
  - 评分行追加 `作者 +N / 顶会 +N / 降权 -N`
  - 新增一行 `- **加分原因**: featured author: Hongyang Li@HKU +3; venue accepted: CVPR +4; blacklist keyword: UAV -6`
...
- 规则预筛：`BLACKLIST_SUBJECTS`
- 排序相关：`MAX_PAPERS_TO_RANK` / `TOPICS_OF_INTEREST` / `RAW_SCORE_MAX` / `TOTAL_SCORE_MAX` / `BONUS_BUDGET`
- 分数叠加：`FEATURED_AUTHORS` / `AUTHOR_BONUS_PER_HIT` / `AUTHOR_BONUS_CAP` / `FEATURED_VENUES` / `VENUE_BONUS` / `BLACKLIST_KEYWORDS` / `PENALTY_PER_HIT` / `PENALTY_CAP`
...
2. **重操作（深度精读）只对少量论文做**；机构推理则覆盖整个 selected 集（粗筛通过的全量候选）
...
13. **新增的评分叠加（作者 / 顶会 / 降权）放在 `score_adjust.py`**，不要改 LLM 输出维度；规则需要配合 `BONUS_BUDGET` 封顶，保证"加分不会盖过内容质量"
14. **featured author 匹配必须"姓名 + 机构"双命中**，避免同名误判；新增 featured author 时给出至少 2 个机构关键词变体
15. **主题黑名单分两层**：彻底不感兴趣的分区走 `BLACKLIST_SUBJECTS` 直接剔除（规则预筛）；边缘但不绝对拒绝的主题走 `BLACKLIST_KEYWORDS` 降权（`score_adjust.py`），保留候选以便 featured author / 顶会信号还能把分抬上来
